    ...
    const fetchProducts = async () => {
      try {
        const params = new URLSearchParams({
          page: "1",
          pageSize: "9", // or "random", "popular" etc.
          sortNewest: "true",
          onlyWithPhotos: "true"
        });
              ...
              <Carousel className="relative w-full px-5 mx-5 max-w-10/12">
                <CarouselContent>
                  {products.map((product, i) => (
                    ...
                    <motion.div
                      key={product.productId}
                      className="bg-white rounded-xl shadow-md overflow-hidden transition-all cursor-pointer mute"
                      initial={{ opacity: 0, y: 30 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ }}
                    >
                      <ProductOverview animate className="bg-gray-100 hover:bg-white transition-all duration-200" key={product.productId} {...product} isEmployee={isEmployee()} addToCartCallback={addToCartCallback} />
                    </motion.div>
                    </CarouselItem>
              ))}
                </CarouselContent>
